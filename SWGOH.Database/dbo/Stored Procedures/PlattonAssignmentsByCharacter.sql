...
-- =============================================
CREATE PROCEDURE [dbo].[PlattonAssignmentsByCharacter]
(
  ...
  left join TerritoryBattles as tb on tb.Id = tbp.TerritoryBattle_Id
   ...
   select DisplayName, NeedCount, HaveCount
		  , (select top (NeedCount) m.DisplayName  + ','  
			 ...
			 order by stars, Gear, Level, NewId()
			 For XML PATH ('')) AS AssignedMembers
			 ,(select pt.PhaseLocation + ' ' + Cast(PlatoonNumber as varchar(1)) + ','
			From TerritoryPlatoons as tp
			left join PhaseTerritories as pt on pt.id = tp.PhaseTerritory_Id
			left join TerritoryBattlePhases as tbp on tbp.Id = pt.TerritoryBattlePhase_Id
			left join PlatoonCharacters as pc on pc.TerritoryPlatoon_Id = tp.Id
			WHERE tbp.Id = @phaseGuid
			and pc.Character_Id = CharId
			and tp.IsClosed = 0
			Order by PhaseLocation desc, PlatoonNumber
			For XML PATH ('')) as Platoons
